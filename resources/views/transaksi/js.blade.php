...
$.ajax({
        ...
        success: function(data){
            if (data.status == true)
            {
              ...
              $('#AjukanModal .modal-body #biaya').val(data.matrik.total_biaya)
              if (data.hasil.form_nomor_surat)
              {
                //ada isiannya
                $('#AjukanModal .modal-body #form_nomor_surat').val(data.hasil.form_nomor_surat)
              }
              else 
              {
                var Tanggal = new Date();
                var srttugas = 'B-'+ data.hasil.trx_id +'/BPS/'+ data.matrik.unit_pelaksana +'/'+ (Tanggal.getMonth()+1) + '/' + Tanggal.getFullYear();
                $('#AjukanModal .modal-body #form_nomor_surat').val(srttugas)
              }
              
              $('#AjukanModal .modal-body #form_tgl_surat').val(data.hasil.form_tgl_surat)
              ...
              $('#AjukanModal .modal-body #peg_nip').val(data.pegawai.hasil_pegawai.peg_nip)
            }
            else 
            {
                alert(data.hasil);
...
$.ajax({
        ...
        success: function(data){
            if (data.status == true)
            {
              ...
              {
                //ada isiannya
                $('#EditModal .modal-body #form_nomor_surat').val(data.hasil.form_nomor_surat)
              }
              else 
              {
                var Tanggal = new Date();
                var srttugas = 'B-'+ data.hasil.trx_id +'/BPS/'+ data.matrik.unit_pelaksana +'/'+ (Tanggal.getMonth()+1) + '/' + Tanggal.getFullYear();
                $('#EditModal .modal-body #form_nomor_surat').val(srttugas)
              }
              
              $('#EditModal .modal-body #editform_tgl_surat').val(data.hasil.form_tgl_surat)
              ...
              $('#EditModal .modal-body #peg_nip').val(data.pegawai.hasil_pegawai.peg_nip)
