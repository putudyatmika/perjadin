...
@if ($sm==1)
<button type="button" class="btn btn-danger btn-rounded btn-sm" data-toggle="modal" data-target="#KonfirmasiKabidModal" data-flagkabid="{{$item->kabid_konfirmasi}}" data-flagket="{{$item->kabid_ket}}" data-tujuan="{{$item->Matrik->kodekab_tujuan}}-{{$item->Matrik->Tujuan->nama_kabkota}}" data-sm="{{$item->Matrik->dana_unitkerja}}-{{$item->Matrik->DanaUnitkerja->nama}}" data-biaya="@rupiah($item->Matrik->total_biaya)" data-infotgl="{{$item->Matrik->tgl_awal}} s/d {{$item->Matrik->tgl_akhir}}" data-trxid="{{$item->trx_id}}" data-kodetrx="{{$item->kode_trx}}" data-kodetransaksi="{{$item->kode_trx}}" data-lamanya="{{$item->Matrik->lamanya}}" data-pegawai="{{$item->TabelPegawai->nip_baru}}-{{$item->TabelPegawai->nama}}" data-tglbrkt="{{$item->tgl_brkt}}" data-tglbalik="{{$item->tgl_balik}}" data-matrikid="{{$item->matrik_id}}" data-tugas="{{$item->tugas}}" data-sumberdana="{{$item->Matrik->dana_mak}} - {{$item->Matrik->DanaAnggaran->uraian}}" data-tid="{{$item->Matrik->dana_tid}}" data-pagurencana="{{$item->Matrik->total_biaya}}" data-makid="{{$item->Matrik->mak_id}}">SM</button>
@endif
@if ($ppk==1)
